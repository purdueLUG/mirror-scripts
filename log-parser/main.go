...
func gatherMetricsFromLog() {
        ...
        rsyncerrorline := 2 //How many lines back the rsync error will occur at

        for i := range rsynclogs {
                logfile, err := os.ReadFile(rsynclogdir + rsynclogs[i].Name())
                herr(err, rsynclogdir+rsynclogs[i].Name())
                logfilelines := strings.Split(string(logfile), "\n")
                if len(logfilelines) > rsyncerrorline {
                        if strings.Contains(logfilelines[len(logfilelines)-rsyncerrorline], "rsync error") { //If the log file contains the evil words
                                actuallyanerror := true
                                for j, _ := range ignoredrsyncerrors {
                                        if strings.Contains(logfilelines[len(logfilelines)-rsyncerrorline], j) {
                ...
                if rsynclogs[i].Size() == 0 { //If the log file is of size 0
